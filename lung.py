...
## 4. 산점도 시각화
    ...
    # 🌟 [수정] 다운로드 받으신 OTF 파일명으로 지정 (대소문자 주의)
    font_path = "NanumGothic.otf"  
    
    if os.path.exists(font_path):
        # OTF 폰트를 matplotlib 시스템에 강제로 등록합니다.
        fm.fontManager.addfont(font_path)
        prop = fm.FontProperties(fname=font_path)
        plt.rcParams['font.family'] = prop.get_name()
    else:
        # 파일명을 못 찾을 때를 대비한 안전장치
        st.error(f"🚨 현재 폴더에서 `{font_path}` 파일을 찾을 수 없습니다! 파일명이 정확히 일치하는지 확인해주세요.")
    ...
    cbar.set_label('군집 (Cluster)')
    
    # 새 환자 표시 (X 기호)
    ax.scatter(Smokes, Alkhol, c='red', s=300, marker='X', label='New Patient')
    
    # 라벨 및 타이틀 설정
    ax.set_title('환자 군집 분포도 (흡연량 vs 음주량)', fontsize=14, pad=15)
